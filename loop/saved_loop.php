...
$saved_sql = "SELECT * FROM saved WHERE user_id = '$user_id'";

$saved_result = mysqli_query($conn, $saved_sql);

if (mysqli_num_rows($saved_result) > 0) {

    while ($saved = mysqli_fetch_assoc($saved_result)) {

        $save_id = $saved['post_id'];  

        $post_sql = "SELECT * FROM posts WHERE id = '$save_id' ORDER BY time DESC";  
