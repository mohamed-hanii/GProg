...
$nav_img_sql = "SELECT * FROM users WHERE username = '$username'";

$nav_result = mysqli_query($conn, $nav_img_sql);

$user_nav = mysqli_fetch_assoc($nav_result);

$nav_profile_img = $user_nav['profile_img'];
...
$user_answers_sql = "SELECT * FROM `comment`WHERE user_id = '$user_id'";

$user_answers_result = mysqli_query($conn, $user_answers_sql);

$answers_num = $user_answers_result -> num_rows ;
